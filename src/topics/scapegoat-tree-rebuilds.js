  ...
  sections: [
    ...
    },
    {
      ...
      paragraphs: [
        ...
        `Once the scapegoat is found, rebuild its entire subtree. Traverse the subtree inorder to collect nodes into a sorted array. Then recursively pick medians: the middle element becomes the root, the left half becomes the left subtree, the right half becomes the right subtree. Reattach the rebuilt subtree to the scapegoat parent. The result is a perfectly balanced subtree.`,
    ...
    },
    {
      ...
      paragraphs: [
        {type: 'bullets', items: [
          'Scapegoat tree: no per-node metadata. Search is worst-case O(log n). Insert and delete are O(n) worst case but O(log n) amortized. Rebalancing is a full subtree rebuild, and the height bound is log_{1/alpha}(n).',
          'AVL tree: stores a balance factor or height. Search, insert, and delete are worst-case O(log n). Rebalancing uses single or double rotations, with a tight height bound near 1.44 log n.',
          'Red-black tree: stores a color bit. Search, insert, and delete are worst-case O(log n). Rebalancing uses rotations plus recoloring, with height bounded by about 2 log n.',
          'Treap: stores a random priority. Search, insert, and delete are expected O(log n), with O(n) possible under unlucky or adversarial priority choices. Rebalancing follows heap-priority rotations.',
        ]},
