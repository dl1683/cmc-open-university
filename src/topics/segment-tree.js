...
export const article = {
  sections: [
    ...
    },
    {
      heading: `Study next`,
      paragraphs: [
        `Return to Fenwick Tree (Binary Indexed Tree) to see why it works for invertible combines and how its smaller code trades off range-query power. Study Binary Heap (Priority Queue) to understand how other tree-based structures manage order. Explore Merge Sort to solidify divide-and-conquer thinking and see where the 2·log₂(n) bound comes from. Master Memoization (Dynamic Programming) to contrast deferred computation (lazy tags) against cached results (memoization) — they are mirrors. Review Binary Search Tree for tree-structure intuition and range ownership. Together, these topics unlock the full toolkit for interval queries, range updates, and the algebra of associative operations that power databases, compilers, and every system doing statistics at scale.`,
      ],
    },
  ],
};
