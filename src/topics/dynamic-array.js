  ...
  sections: [
    {
      ...
      paragraphs: [
        'The animation shows the backing array — the fixed-size block of memory that the dynamic array owns. Slots with values are live data; slots shown as empty (null) are allocated capacity waiting to be filled. The label below tracks two numbers: size (how many elements are stored) and capacity (how many slots exist in the backing buffer).',
        ...
        'When an append fits within existing capacity, the new value appears in the next empty slot and size increments by one. This is a single O(1) write — no copying, no allocation.',
        'When the array is full (size equals capacity), a resize fires. The animation highlights all existing elements, allocates a new array with double the capacity, copies every element into it, and then writes the new value. Watch the capacity jump: 2 to 4, 4 to 8, 8 to 16. Each resize is expensive, but the gaps between resizes keep growing — that is the source of the amortized cost.',
    ...
    },
    {
      ...
      paragraphs: [
        'A plain array has a fixed size set when it is created. If you know exactly how many elements you need, that is fine. But most real programs do not know in advance: a user types search results, a server accumulates log entries, a parser builds a token list. The program needs an array that grows on demand while still providing O(1) random access by index.',
    ...
    },
    {
      ...
      paragraphs: [
        ...
        'The root cause is that the grow-by-one strategy earns zero future benefit from each copy. You pay n operations to copy n elements into a new array, and the very next append requires another copy of n+1 elements. The expensive work never buys any breathing room.',
      ],
    },
    {
      ...
      paragraphs: [
        ...
        'Doubling is the key. It makes copies exponentially rarer. The first resize copies 2 elements. The next copies 4. Then 8, then 16. But between each resize, you get 2, 4, 8, 16 free appends respectively. The expensive operations and the cheap ones grow at the same rate, so the average cost per append stays constant.',
      ],
    },
    {
      heading: 'How it works',
      paragraphs: [
        'The dynamic array stores three things: a pointer to the backing array, the current size (number of live elements), and the current capacity (total allocated slots).',
        'Append: if size < capacity, write the value at index size and increment size. Cost: O(1). If size == capacity, allocate a new array of capacity 2 * capacity, copy all size elements into it, release the old array, write the new value, and increment size. Cost: O(n) for this one append.',
        'Random access: return backing[index]. Cost: O(1), identical to a plain array because the backing store is contiguous.',
        ...
        'Delete at index i: shift elements i+1 through size-1 one position left, decrement size. Cost: O(n - i). Some implementations shrink the backing array when size drops below capacity/4 to reclaim memory, using the same doubling/halving discipline.',
      ],
    },
    {
      ...
      paragraphs: [
        ...
        'The potential function method gives the same result more formally. Define P = 2 * size - capacity (measured after the most recent resize). A cheap append increases size by 1, so P rises by 2; amortized cost = actual cost (1) + change in potential (2) = 3. A resize of n elements has actual cost n, but capacity doubles and size increments by 1, so P drops by roughly n; amortized cost = n + (drop of ~n) = O(1). Either way, each append is O(1) amortized.',
      ],
    },
    {
      ...
      paragraphs: [
        ...
        'Random access (read or write by index): O(1). The backing array is contiguous, so index arithmetic is a single pointer addition, identical to a plain array.',
        ...
        'Insert at index i: O(n - i). Elements after the insertion point shift right. Inserting at the front is O(n). Inserting at the end is append, O(1) amortized.',
        ...
        'Space: at most 2x the number of stored elements. Right after a resize, size = capacity/2, so half the memory is unused. Right before a resize, size = capacity, so nothing is wasted. On average, about 75% of allocated space holds real data.',
        'The hidden constant: resizing requires allocating a new contiguous block and copying everything. For a 1 GB array, that means finding a 2 GB block and copying 1 GB of data. The allocator must locate contiguous space, and the copy touches every cache line. This is why some systems pre-allocate or use chunked structures for very large collections.',
      ],
    },
    {
      heading: 'Real-world uses',
      paragraphs: [
        'Java ArrayList: the default growable list. Uses 1.5x growth factor instead of 2x, trading slightly more frequent resizes for less wasted space. The amortized O(1) guarantee holds with any constant factor greater than 1.',
        'Python list: every Python list is a dynamic array. list.append() is O(1) amortized. Python uses a growth formula roughly 1.125x for large lists, optimized for memory over resize frequency.',
        'C++ std::vector: the workhorse container. Cache-friendly because elements are contiguous in memory. The standard guarantees amortized O(1) push_back. Most implementations double capacity.',
        'JavaScript arrays: V8 and other engines implement dense arrays as dynamic arrays internally. Array.push() is O(1) amortized.',
        'Go slices: a slice is a view into a backing array with length and capacity. append() doubles capacity when full (for small slices) and grows by roughly 1.25x for large ones.',
        'The pattern also powers hash tables. When a hash table\'s load factor crosses a threshold, it allocates a larger bucket array (typically 2x) and rehashes every entry. The same amortized argument applies: rehashing is O(n) but happens rarely enough that each insert is O(1) amortized.',
      ],
    },
    {
      ...
      paragraphs: [
        ...
        'Memory fragmentation. A resize allocates a new block of 2x the old size and frees the old block. The old block\'s memory may not be reusable for the new larger allocation. For very large arrays, this can cause allocation failure even when total free memory is sufficient, because no single contiguous block is large enough.',
        'Wasted space. Right after a resize, half the capacity is empty. If the array grows to just past a power of two and then stops, nearly half the memory sits unused. C++ shrink_to_fit() and Java trimToSize() reclaim this, but they trigger another full copy.',
        'No stable pointers. Every resize moves all elements to new memory. Pointers or references to elements become invalid after any append that triggers a resize. C++ iterators and pointers into a vector are invalidated on push_back when capacity changes.',
      ],
    },
    {
      ...
      paragraphs: [
        ...
        'Total cost for 5 appends: 1 + 1 + 3 + 1 + 5 = 11. Two resizes occurred at sizes 2 and 4, copying 2 and 4 elements respectively. Average cost per append: 11/5 = 2.2. Even in this small example, the amortized cost is well under 3. As n grows, the average approaches 3 (with growth factor 2) because the O(1) appends between resizes dominate more and more.',
      ],
    },
    {
