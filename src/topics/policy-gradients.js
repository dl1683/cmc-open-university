...
// ∇θ log π(a): for a softmax, it's (1{a=i} − π(i)) per logit i.
const scoreVec = (pi, a) => [a === 0 ? 1 - pi[0] : -pi[0], a === 1 ? 1 - pi[1] : -pi[1]];
// True gradient of E[R]: Σ_a π(a)·R(a)·∇log π(a).
...
// Sample N episodes at θ = (0, 0) and estimate the θ₁-gradient with and
...
// for advantage +1 and −1, ε = 0.2 — the famous pair of curves.
const EPS = 0.2;
...
const PPO_NEG = { id: 'aneg', label: 'advantage −1 (bad action)', points: rGrid.map((r) => ({ x: r, y: clipObj(r, -1) })) };
...
function* reinforce() {
  yield {
    state: table('The problem: reward arrives AFTER a dice roll', [
      ...
      ['policy π(a|θ) = softmax(θ) picks an action; the world pays R — here action 0 pays +1, action 1 pays +3'],
      ['sampling an action is a discrete jump, and the reward comes from outside the program: no chain rule through either'],
      ['∇θ E[R] = E[ R · ∇θ log π(a) ] — differentiate the PROBABILITY of what you did, weight it by what you got'],
...
function* ppo() {
  ...
  yield {
    state: plotState({
      axes: { x: { label: 'r = π_new(a) / π_old(a)', min: 0, max: 2 }, y: { label: 'clipped objective', min: -2.1, max: 1.4 } },
      series: [PPO_POS, PPO_NEG],
    }),
    highlight: { compare: ['apos', 'aneg'] },
    explanation: 'PPO\'s entire idea, drawn live from its formula: L = min(r·A, clip(r, 0.8, 1.2)·A), where r is the importance ratio π_new/π_old for an action and A its advantage. Read the good-action curve (A = +1): the objective rewards raising r — but FLATLINES at r = 1.2. Beyond that, zero gradient: no further credit for pushing a good action harder than 20% past the old policy, so the incentive to overshoot simply vanishes. The bad-action curve (A = −1) is the mirror with a twist: it flatlines at r = 0.8 going down, yet stays STEEP for r > 1 — if a bad action somehow got MORE likely, the gradient to undo the mistake is never clipped. Pessimism in both cases: the min() always picks whichever term hurts the update more.',
    invariant: 'The clip kills gradients that push r beyond [1−ε, 1+ε] in the favorable direction — and never clips the correction.',
  };

  yield {
    state: table('Why the clip ≈ a trust region (and what it costs)', [
      ['reuse', 'data reuse'],
      ...
      ['flat objective beyond r = 1 ± ε ⇒ near-zero incentive to leave the neighborhood: TRPO\'s KL ball, enforced by indifference instead of constraint'],
  ...
  yield {
    state: table('The lineage, and where it runs today', [
      ...
      ['a learned value function as the baseline: the advantage A = R − V(s), variance tamed at scale'],
    ...
    invariant: 'From Williams to GRPO, one identity and one variance fix: ∇E[R] = E[(R − b)·∇log π] — everything else is engineering.',
  };
}
  ...
  sections: [
    {
      heading: `What it is`,
      paragraphs: [
        `Policy gradients are the foundational method for learning when you cannot backpropagation through the world. Instead of differentiating a reward function (which doesn't exist), you differentiate the *probability* of what you did and weight it by what you got. The core identity is ∇E[R] = E[R·∇log π], the score-function trick: "increase the log-probability of actions that led to good outcomes, decrease it for bad ones." No differentiable environment. No simulator. No model of the world — only samples: action you took, reward you received. REINFORCE (Williams, 1992) applied this to neural networks and changed reinforcement learning forever.`,
      ],
    },
    {
      heading: `How it works`,
      paragraphs: [
        `The score-function identity works because the gradient of a log-probability tells you how to nudge its argument to make the event MORE likely. For a softmax policy over actions, ∇log π(a) points in the direction of higher probability for action a. If you scale it by the reward you got, you're saying: "this action got +3, so turn UP its probability; that other action got +1, so barely nudge it; a failed action got −2, so push its probability DOWN." This is exactly what humans do intuitively in games: repeat what worked, avoid what didn't. The mathematics simply formalizes this instinct as a gradient.`,
        `The visualization shows a two-action world with a simple truth: action 0 pays +1, action 1 pays +3. Noise of ±2 is added because the real world never pays twice the same way. The TRUE gradient has a closed form, so raw REINFORCE estimates can be audited live. With 400 sampled episodes at θ = (0, 0), the raw estimator (no baseline) aims at 0.500 but has noise: mean 0.535, variance 1.28. Now add a baseline b = 2 (the average reward) and recompute advantage = R − b for each sample. The per-episode estimate shrinks: same mean 0.500, variance drops to 0.35 — a 3.6× cut with zero bias, because E[b·∇log π] = 0 always (probabilities sum to one). Training curves (60 updates, 8 episodes, identical seed) show both approaches learning the right policy, but the baseline curve climbs smoothly while the no-baseline curve lurches visibly — one bad luck streak and a bad batch wrong-steps forward.`,
      ],
    },
    {
      heading: `Cost and complexity`,
      paragraphs: [
        `REINFORCE is O(1) per sample: compute the policy, sample an action, receive a reward, scale ∇log π, update. In practice, the complexity is hidden in the batch size and episode length: longer environments = more steps to collect before updating. The true cost is *variance*. The baseline solves it but only partway; moving to a learned value function (actor-critic) takes variance much lower by learning to predict cumulative future reward, not just rescaling past reward. PPO tackles a different cost: the step-size cliff. A policy is a distribution, not a point in parameter space. One huge gradient step can drop a good action from 60% probability to 5% — tiny in θ-space but catastrophic in distribution space, measured by KL divergence. TRPO (2015) solved this with natural gradient and an explicit KL trust region (heavy, second-order). PPO (2017) replaces the constraint with a clipped objective L = min(r·A, clip(r, 0.8, 1.2)·A) where r = π_new / π_old: if you push a good action too hard, the objective flatlines and stops rewarding further progress. No constraint, no Hessian, one simple clip per action — this is why PPO trains modern LLMs.`,
      ],
    },
    {
