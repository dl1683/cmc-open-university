...
// another, two subtractions cancel everything fixed about places and times â€”
...
export const topic = {
  ...
  category: 'Concepts',
  summary: 'Subtract across time to kill place effects, subtract across places to kill time effects: the 2Ã—2 that recovers a known policy effect live â€” and the parallel-trends bet it rides on.',
  controls: [
...
// A synthetic minimum-wage world (Cardâ€“Krueger shaped), fully known:
//   employment = base(state) + trend(period) + EFFECTÂ·(treated & after)
// base: NJ 78, PA 92 (PA permanently bigger â€” a level difference that must
...
function* twoByTwo() {
  yield {
    state: table('1992: New Jersey raises its minimum wage. Pennsylvania does not.', [
      ...
      [`${NJ_AFTER} âˆ’ ${NJ_BEFORE} = ${D_NJ}: employment FELL after the raise â€” but a recession hit everyone that year, and this difference carries it`],
      [`${NJ_AFTER} âˆ’ ${PA_AFTER} = ${NJ_AFTER - PA_AFTER}: NJ employs fewer than PA â€” but NJ was always smaller; this difference carries the permanent gap`],
    ...
    explanation: `The setting is the most famous natural experiment in economics (Card & Krueger, 1994): New Jersey raised its minimum wage while next-door Pennsylvania didn't â€” and this page rebuilds it as a synthetic world where the policy's TRUE effect on employment is exactly +${TRUE_EFFECT}, so every estimator can be audited. Both obvious comparisons fail, and the table computes how: comparing NJ to itself over time (${D_NJ}) blames the policy for that year's recession; comparing NJ to PA after the fact (${NJ_AFTER - PA_AFTER}) blames it for a size gap that predates the policy by decades. One difference is never enough when two kinds of nuisance â€” fixed PLACE effects and shared TIME effects â€” are both in the data.`,
  ...
  yield {
    state: table(`The 2Ã—2, fully computed (truth = +${TRUE_EFFECT})`, [
      ...
      ['â€”', 'â€”', `${D_NJ} âˆ’ (${D_PA}) = +${DID} â€” the policy effect, exactly, with both nuisances dead`],
    ]),
    highlight: { found: ['diff:delta'], compare: ['nj:delta', 'pa:delta'] },
    explanation: `Four numbers, two subtractions, computed live. Subtract WITHIN each state across time: NJ changed by ${D_NJ}, PA by ${D_PA}. The within-state subtraction already killed the level gap â€” NJ's permanent smallness (${BASE.nj} vs ${BASE.pa}) appears in both of NJ's cells and cancels; likewise PA's. What remains in each change is (shared recession) plus, for NJ only, the policy. Now subtract ACROSS states: the recession (${TREND}) sits identically in both changes and cancels, leaving ${D_NJ} âˆ’ (${D_PA}) = +${DID} â€” the true effect to the digit. Anything constant about a place dies in the first difference; anything shared about a time dies in the second. The estimator is literally its own name.`,
  ...
  yield {
    ...
    explanation: `The same arithmetic as a picture â€” the picture every DiD paper draws. PA's line falls with the recession. The dashed-role third line is the COUNTERFACTUAL: where NJ would have gone with no policy, built by giving NJ's starting point PA's slope. The vertical gap between NJ's actual endpoint (${NJ_AFTER}) and that counterfactual endpoint (${NJ_BEFORE + D_PA}) is the estimate: +${DID}. Said plainly: the control group's job is to stand in for the treated group's missing timeline. The whole design is one assumption wearing a graph â€” that NJ, untreated, would have moved PARALLEL to PA â€” and the next view stress-tests exactly that.`,
...
function* parallelTrends() {
  yield {
    state: table('The load-bearing assumption: PARALLEL TRENDS', [
      ...
      ['absent the policy, treated and control would have CHANGED by the same amount â€” levels may differ forever; SLOPES must match'],
      ['groups need not be similar! NJ â‰  PA in size, industry, everything â€” DiD only borrows PA\'s slope, never its level (that died in the first subtraction)'],
      ['plot several PRE-policy periods: if the two lines moved in lockstep before treatment, the lockstep claim after is more credible â€” the "event-study" plot every referee demands'],
      ['necessary, not sufficient: parallel HISTORY is evidence, not proof, about the counterfactual FUTURE â€” the assumption is untestable exactly where it matters, like exclusion in Instrumental Variables & Natural Experiments'],
    ]),
    highlight: { active: ['nice:detail'], removed: ['evidence:detail'] },
    explanation: 'Everything DiD buys is purchased with one assumption, so state it precisely. Parallel trends does NOT say the groups are comparable in level â€” the design\'s superpower is that permanent differences cancel for free. It says: had the policy never happened, the treated group\'s CHANGE would have equaled the control\'s. That is a claim about a world that never ran, so it cannot be verified â€” only made plausible. The standard evidence is pre-trends: show the two lines marching in parallel for several periods before treatment. Lockstep history makes lockstep counterfactuals credible; it cannot make them certain. Every DiD argument is ultimately an argument about why the control\'s slope deserves to be borrowed.',
    invariant: 'DiD\'s identifying assumption lives in slopes, not levels â€” testable in the past, assumed in the counterfactual, never proven.',
  };
  ...
  yield {
    state: table('How it breaks: three classic failure stories', [
      ...
      ['a casino opens in PA the same year â€” PA\'s slope steepens for reasons NJ doesn\'t share', 'the borrowed slope is wrong; the casino\'s boost gets subtracted from NJ and masquerades as policy damage'],
      ['legislatures raise minimum wages WHEN local employment is already trending up (or down) â€” treatment timing correlates with the outcome\'s slope', 'the pre-existing trend differential loads straight into the estimate: the policy gets credit (or blame) for momentum it never caused'],
    ...
    explanation: 'The three ways real DiD studies die. A LOCAL SHOCK to the control (anything that bends PA\'s slope alone) corrupts the borrowed counterfactual â€” and note the cruelty: the better your control resembles a real economy, the more things can happen to it. TARGETING is subtler: policies are passed by people watching the outcome, so treatment timing is correlated with trends by construction â€” the exact selection-on-trends that parallel trends forbids. ANTICIPATION moves the response across the before/after line itself. None of these announce themselves in the 2Ã—2; all of them show up â€” sometimes â€” as pre-trend divergence, which is why the event-study plot is the genre\'s mandatory first figure.',
    invariant: 'Anything that bends ONE group\'s slope â€” shocks, targeting, anticipation â€” loads directly into the estimate, disguised as the policy.',
  };
  ...
  yield {
    state: table('The toolkit, complete: four designs, one question', [
      ...
      ['you control assignment â€” A/B Testing & p-values', 'none: the coin severs every arrow (the gold standard, when ethics and logistics allow)'],
      ["confounders are MEASURED â€” Causal Graphs, Confounding & Simpson's Paradox + Doubly Robust Estimation", 'you drew the right graph and measured every back door'],
      ['confounders unmeasured, but something random nudged treatment â€” Instrumental Variables & Natural Experiments', 'exclusion: the lottery touches the outcome through one door only'],
      ['confounders unmeasured, but a policy hit one group and time hit both â€” this page', 'parallel trends: the control\'s slope is the treated group\'s missing future'],
    ]),
    highlight: { active: ['did:bet'] },
    explanation: 'The observational-causality toolkit, now complete on this site, lined up by the bet each design asks you to make. Randomization buys certainty with control; adjustment buys reach with a graph you must defend; instruments buy unmeasured-confounder immunity with an exclusion argument; and DiD buys it with a slope-borrowing argument â€” often the cheapest bet available, because policies that hit one jurisdiction and not its neighbor are everywhere once you look. Card & Krueger\'s original found employment did NOT fall (their DiD came out slightly positive) â€” a result that overturned textbook consensus and eventually shared in a Nobel prize. One 2Ã—2 table, argued carefully, can do that.',
...
export const article = {
  sections: [
    ...
    {
      heading: `The obvious comparisons`,
      paragraphs: [
