...
export const article = {
  sections: [
    ...
    },
    {
      heading: `Study next`,
      paragraphs: [
        `Read "A/B Testing & p-values" to understand why randomization is the gold standard for causal inference — it severs every incoming arrow to treatment, eliminating confounding by design. Study "Doubly Robust Estimation" to see how propensity weighting combines the backdoor rule with modern estimation, protecting against violations. Explore "Importance Sampling & Off-Policy Estimation" to learn how reweighting deals with off-policy data, a causal problem in reinforcement learning. Master "Naive Bayes (Spam Filter)" to see graphical-model thinking applied to a simple classification problem: directed arrows represent independence assumptions that make computation tractable. Together, these topics build the full causal toolkit: graphs tell you what to adjust for, A/B tests let you avoid adjustment altogether, Doubly Robust methods and importance sampling make adjustment robust, and graphical models show how causation underlies probabilistic inference.`,
      ],
    },
  ],
};
