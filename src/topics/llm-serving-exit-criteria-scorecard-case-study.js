...
export const article = {
  sections: [
    { heading: 'How to read the animation', paragraphs: [
      'Read this as a release gate for a large language model (LLM) serving change. Active nodes are gates under review, compare nodes are the old path, and found nodes are evidence strong enough to ship.',
      {type: 'callout', text: 'A serving optimization ships only when every gate proves benefit, contained failure, and rollback.'},
    ] },
    { heading: 'Why this exists', paragraphs: [
      'Serving teams tune batching, prefix caching, CUDA graphs, quantization, speculation, and routing because each can lower cost. Each lever can also move cost into latency, quality, reliability, or rollback risk.',
    ] },
    { heading: 'The obvious approach', paragraphs: [
      'The obvious approach is local benchmarking. If batching changed, measure tokens per second; if prefix caching changed, measure hit rate; if quantization changed, measure cost per token.',
    ] },
    { heading: 'The wall', paragraphs: [
      'The wall is metric coupling. A throughput gain can increase p99 latency, a cache gain can increase memory pressure, and a cheaper dtype can fail protected quality slices.',
    ] },
    { heading: 'The core insight', paragraphs: [
      'A scorecard is a release packet, not a dashboard. Each gate pairs a claimed benefit with the failure mode that could invalidate the release.',
    ] },
    { heading: 'How it works', paragraphs: [
      'The packet starts with a hypothesis, blast radius, slice list, and rollback plan. It then runs load gates, quality gates, lever-specific checks, canary gates, and an exercised rollback before widening traffic.',
    ] },
    { heading: 'Why it works', paragraphs: [
      'The correctness argument is invariant checking across product boundaries. A change can ship only if protected slices stay inside latency, quality, error, cost, and rollback bounds while the intended benefit appears.',
    ] },
    { heading: 'Cost and complexity', paragraphs: [
      'The cost is release friction: traffic generators, evals, slice metrics, canary routing, rollback flags, stored artifacts, and owners. Cost should scale with blast radius, so a private experiment needs a lighter packet than a shared multi-tenant platform.',
    ] },
    { heading: 'Real-world uses', paragraphs: [
      'Scorecards fit shared vLLM, TensorRT-LLM, Triton, Ray Serve, and custom inference platforms. They help when one serving change touches many tenants, models, GPU types, prompt lengths, and risk classes.',
    ] },
    { heading: 'Where it fails', paragraphs: [
      'The scorecard fails when gates are decorative. Vague thresholds, missing owners, average-only metrics, stale evals, and untested rollback turn release control into paperwork.',
    ] },
    { heading: 'Worked example', paragraphs: [
      'A team raises max batch tokens from 8192 to 16384 for a 70B model. Throughput rises from 1200 to 1680 tokens per second, but p99 first-token latency for 16000-token prompts worsens from 2.8 seconds to 7.4 seconds against a 5 second SLO.',
      'The scorecard blocks the release even though aggregate throughput improved by 40 percent. The long-context slice violates the product contract, so the benefit is not usable production goodput.',
    ] },
    { heading: 'Sources and study next', paragraphs: [
      'Study Orca at https://www.usenix.org/conference/osdi22/presentation/yu, Efficiently Scaling Transformer Inference at https://arxiv.org/abs/2211.05102, NVIDIA Triton Dynamic Batcher at https://docs.nvidia.com/deeplearning/triton-inference-server/user-guide/docs/user_guide/batcher.html, and vLLM prefix caching at https://docs.vllm.ai/en/stable/design/prefix_caching/.',
      'Next, study Continuous Batching, Prefix Caching, CUDA Graphs, Speculative Decoding, Quantization, LLM Unit Economics Ledger, Canary Rollout, Tail Latency and p99 Thinking, and SLO-Aware LLM Request Router.',
    ] },
  ],
};